...
STDMETHODIMP StringProperties::EditProperty(DISPID dispID)
{
	...
	if(m_ppUnk[0]->QueryInterface(IID_IProgressBar, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
...
LRESULT StringProperties::OnLoadSettingsFromFile(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/)
{
	...
	if(m_ppUnk[0]->QueryInterface(IID_IProgressBar, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
		...
		if(dlg.DoModal() == IDOK) {
			...
			reinterpret_cast<IProgressBar*>(pControl)->LoadSettingsFromFile(file, &b);
			if(b == VARIANT_FALSE) {
...
LRESULT StringProperties::OnSaveSettingsToFile(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/)
{
	...
	IUnknown* pControl = NULL;
	if(m_ppUnk[0]->QueryInterface(IID_IProgressBar, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
		CFileDialog dlg(FALSE, NULL, TEXT("ProgressBar Settings.dat"), OFN_ENABLESIZING | OFN_EXPLORER | OFN_CREATEPROMPT | OFN_HIDEREADONLY | OFN_LONGNAMES | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT, TEXT("All files\0*.*\0\0"), *this);
		if(dlg.DoModal() == IDOK) {
			CComBSTR file = dlg.m_szFileName;

			VARIANT_BOOL b = VARIANT_FALSE;
			reinterpret_cast<IProgressBar*>(pControl)->SaveSettingsToFile(file, &b);
			if(b == VARIANT_FALSE) {
...
void StringProperties::ApplySettings(void)
{
	...
	for(UINT object = 0; object < m_nObjects; ++object) {
		LPUNKNOWN pControl = NULL;
		if(m_ppUnk[object]->QueryInterface(IID_IProgressBar, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
			CComBSTR bstr = properties.text;
			reinterpret_cast<IProgressBar*>(pControl)->put_Text(bstr);
			pControl->Release();
		}
	}
...
void StringProperties::LoadSettings(void)
{
	...
	BSTR value;
	//TODO: RightToLeftConstants rtlSettings = static_cast<RightToLeftConstants>(0);
	for(UINT object = 0; object < m_nObjects; ++object) {
		LPUNKNOWN pControl = NULL;
		if(m_ppUnk[object]->QueryInterface(IID_IProgressBar, reinterpret_cast<LPVOID*>(&pControl)) == S_OK) {
			if(m_nObjects == 1) {
				reinterpret_cast<IProgressBar*>(pControl)->get_Text(&value);
				properties.text = value;
				SysFreeString(value);
				//TODO: reinterpret_cast<IProgressBar*>(pControl)->get_RightToLeft(&rtlSettings);
			}
			pControl->Release();
		}
	}
